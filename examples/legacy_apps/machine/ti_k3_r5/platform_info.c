		...
		const struct remoteproc_ops *ops, void *arg)
{
	...
	uint32_t mailbox_id = *((uint32_t *)MAILBOX_BASE_ADDR);
...
//             ;
       ...
       if (MailboxGetMessage(MAILBOX_BASE_ADDR, 0, &msg) == MESSAGE_VALID)
               printf("Reading queue 0: %lu\n", msg);
       if (MailboxGetMessage(MAILBOX_BASE_ADDR, 1, &msg) == MESSAGE_VALID)
               printf("Reading queue 1: %ld\n", (int32_t)msg);
       if (MailboxGetMessage(MAILBOX_BASE_ADDR, 1, &msg) == MESSAGE_VALID)
                      printf("Reading queue 1: %ld\n", (int32_t)msg);
       if (MailboxGetMessage(MAILBOX_BASE_ADDR, 1, &msg) == MESSAGE_VALID)
                      printf("Reading queue 1: %ld\n", (int32_t)msg);
       if (MailboxGetMessage(MAILBOX_BASE_ADDR, 1, &msg) == MESSAGE_VALID)
                      printf("Reading queue 1: %ld\n", (int32_t)msg);
       if (MailboxGetMessage(MAILBOX_BASE_ADDR, 2, &msg) == MESSAGE_VALID)
               printf("Reading queue 2: %lu\n", msg);

       printf("About to kick mailbox\n");

       if (MailboxSendMessage(MAILBOX_BASE_ADDR, 0, -252) == 0)
               printf("Sent on queue 0: %d\n", -252);
	...
	MailboxEnableNewMsgInt(MAILBOX_BASE_ADDR, 0, 1); // 1, 0); ?
